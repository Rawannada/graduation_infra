...
class PDFProcessor:
    def __init__(self, pdf_source):
        """
        pdf_source: ممكن يكون مسار ملف (String) لجهاز روان
        أو BytesIO object للأجهزة بتاعة الأصحاب (Workers)
        """
    ...
    def extract_text_from_pdf(self, start_page: int = 0, end_page: int = None):
        pages_data = []
        try:
            # بفتح المصدر سواء كان مسار أو بايتس
    ...
    def extract_text_with_ocr(self, start_page: int = 0, end_page: int = None):
        pages_data = []
        try:
            # هنا بنقرر هنحول الصور من مسار ولا من بايتس
    ...
    def process_pdf(self, use_ocr=False, use_sections=True, start_page=0, end_page=None):
        ...
        if use_ocr:
            ...
            for ocr_p in ocr_pages:
                exists = next((p for p in all_pages if p["page_num"] == ocr_p["page_num"]), None)
                if not exists or len(exists["text"]) < 100:
                    if exists: all_pages.remove(exists)
