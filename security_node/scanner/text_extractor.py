...
def _extract_csv(file_path: str) -> tuple[str, str | None]:
    import csv
    for encoding in ("utf-8", "utf-16", "latin-1"):
        try:
            parts = []
            with open(file_path, "r", encoding=encoding, errors="replace", newline="") as f:
                # Use sniffer or strict reader; default works for most standard CSVs
                reader = csv.reader(f)
                for row in reader:
                    # Join non-empty cells with a space for a clean, readable text extraction
            ...
            continue
    return "", "CSV extraction failed: could not decode file with any supported encoding."
